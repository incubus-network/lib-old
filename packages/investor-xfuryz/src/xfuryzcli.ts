...
import { XfuryzApi } from './api'
import { xfuryzAddresses } from './constants'
...
const main = async (): Promise<void> => {
  ...
  const xfuryzContractAddress = xfuryzAddresses[0].xfuryz
  const xfuryContractAddress = xfuryzAddresses[0].xfury
  const xfuryzStakingContractAddress = xfuryzAddresses[0].staking
  const liquidityReserveContractAddress = xfuryzAddresses[0].liquidityReserve
  ...
  const circulatingSupply = async () => {
    try {
      const supply = await api.tvl({ tokenContractAddress: xfuryzContractAddress })
  ...
  const totalSupply = async () => {
    try {
      const supply = await api.totalSupply({ tokenContractAddress: xfuryzContractAddress })
  ...
  const stakingTokenBalance = async () => {
    try {
      const balance = await api.balance({
        tokenContractAddress: xfuryContractAddress,
        userAddress,
      })
  ...
  const rewardTokenBalance = async () => {
    try {
      const balance = await api.balance({
        tokenContractAddress: xfuryzContractAddress,
        userAddress,
      })
  ...
  const stake = async (amount: string) => {
    try {
      ...
      const response = await api.deposit({
        contractAddress: xfuryzStakingContractAddress,
    ...
    switch (index) {
      case 0:
        ...
        switch (tokenContract) {
          case 0:
            await approve(xfuryContractAddress, xfuryzStakingContractAddress)
            break
          case 1:
            await approve(xfuryzContractAddress, xfuryzStakingContractAddress)
            break
          default:
            break
        }
      ...
      case 1:
        ...
        switch (tokenContract) {
          case 0:
            await approve(xfuryContractAddress, liquidityReserveContractAddress)
            break
          case 1:
            await approve(xfuryzContractAddress, liquidityReserveContractAddress)
            break
          default:
            break
        }

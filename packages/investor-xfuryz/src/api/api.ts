...
import { xfuryzAbi } from '../abi/xfuryz-abi'
import { xfuryzStakingAbi } from '../abi/xfuryz-staking-abi'
import {
  DefiType,
  MAX_ALLOWANCE,
  tokeManagerAddress,
...
import {
  ...
  WithdrawInput,
  XfuryzAddressesType,
  XfuryzOpportunityInputData,
} from './xfuryz-types'

export * from './xfuryz-types'
...
export type ConstructorArgs = {
  ...
  providerUrl: string
  xfuryzAddresses: XfuryzAddressesType
...
export const transformData = ({
  ...
  return {
    ...
    rewardToken: contractData.xfuryz,
    stakingToken: contractData.xfury,
...
export class XfuryzApi {
  ...
  constructor({
    ...
    this.xfuryzStakingContracts = xfuryzAddresses.map(
      (addresses) => new this.web3.eth.Contract(xfuryzStakingAbi, addresses.staking),
    )
    this.liquidityReserveContracts = xfuryzAddresses.map(
    ...
    this.xfuryzAddresses = xfuryzAddresses
  }
  ...
  private getStakingContract(contractAddress: string): Contract {
    const stakingContract = this.xfuryzStakingContracts.find(
  ...
  async getXfuryzOpportunities() {
    try {
      const opportunities = await Promise.all(
        this.xfuryzAddresses.map(async (addresses) => {
          const stakingContract = this.xfuryzStakingContracts.find(
            (item) => toLower(item.options.address) === toLower(addresses.staking),
          )
          try {
            const expired = await stakingContract?.methods.pauseStaking().call()
            const tvl = await this.tvl({ tokenContractAddress: addresses.xfuryz })
            const apy = this.apy()
      ...
      throw new Error(`getXfuryzOpportunities Error: ${e}`)
    }
  }

  async getXfuryzOpportunityByStakingAddress(stakingAddress: string) {
    this.verifyAddresses([stakingAddress])
    const addresses = this.xfuryzAddresses.find(async (item) => {
  ...
  async tvl(input: TokenAddressInput): Promise<BigNumber> {
    ...
    this.verifyAddresses([tokenContractAddress])
    const contract = new this.web3.eth.Contract(xfuryzAbi, tokenContractAddress)
  ...
  async getRebaseHistory(input: BalanceInput) {
    ...
    const xfuryzContract = new this.web3.eth.Contract(xfuryzAbi, tokenContractAddress)
    const fromBlock = 14381454 // genesis rebase

    const rebaseEvents = await (async () => {
      try {
        const events = (
          await xfuryzContract.getPastEvents('LogRebase', {
            fromBlock,
            toBlock: 'latest',
          })
    ...
    const transferEvents = await (async () => {
      try {
        const events = await xfuryzContract.getPastEvents('Transfer', {
          fromBlock,
          toBlock: 'latest',
        })
        return events
      ...
      events.map(async (event) => {
        const { preRebaseBalance, postRebaseBalance } = await (async () => {
          try {
            ...
            const postRebaseBalanceResult = await xfuryzContract.methods
              .balanceOf(userAddress)
              .call(null, event.blockNumber)
            const unadjustedPreRebaseBalance = await xfuryzContract.methods
              .balanceOf(userAddress)
    ...
    const actualResults = results.reduce<RebaseHistory[]>((acc, cur) => {
      if (cur.status === 'rejected') {
        console.error('getXfuryzRebaseHistory: balanceOf call failed - charts will be wrong')
        return acc
      }
